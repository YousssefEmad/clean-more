...
$(document).ready(function () {
  $('.success-btn').on('click', function (e) {
      ...
      setTimeout(function () {
        $('#success-model').removeClass('show').addClass('hide');
...
(function ($) {
  ...
  var toolbarBottom = function () {
    var $toolbarBottom = $("#toolbarBottom");
    var $borderProgress = $(".border-progress");
    ...
    $(window).on("scroll", function () {
      ...
      if (scrollTop > 100) {
        $toolbarBottom.addClass("show");
      } else {
        $toolbarBottom.removeClass("show");
      }
    });

    $toolbarBottom.on("click", function () {
      $("html, body").animate({ scrollTop: 0 }, 0);
    });
  };
...
var totalVariant = function () {
  ...
  var currentPrice = price || parseFloat(scope.find(".tf-variant-item-price .price").text().replace("EGP", ""));
  var quantity = parseInt(scope.find("input").val());
  var totalPrice = currentPrice * quantity;
  scope.find(".tf-variant-item-total .price").text(
    totalPrice.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",") + " EGP"
  );
}
